  ...
  .then(function(){
    var request = {
      'samlp:AuthnRequest': {
        ...
        '@ProtocolBinding': self.options.protocol || 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST',
...
SAML.prototype.validatePostResponse = function (container, callback) {
  ...
  zlib.inflateRaw(new Buffer(container.SAMLResponse, 'base64'), function(err, response){
    if(err) return callback(err);
    var xml = response.toString('utf8');
